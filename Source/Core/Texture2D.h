...
namespace Hydro
{
    ...
    {
    public:
        Texture2D(std::string Name, uint32_t Width, uint32_t Height, uint32_t Slot = 0);
        virtual ~Texture2D() = default;
        

        static Ref<Texture2D> Create(const std::string& Name, uint32_t Width, uint32_t Height, uint32_t Slot = 0);
        static Ref<Texture2D> CreateFromFile(const std::string& Name, const Path& Filepath, uint32_t Slot = 0);
